...
<option>Brilliant011</option><option>Transgender</option><option>Align</option><option>Other</option></select>
...
<option>GSH J52</option><option>Philippi Village</option><option>Other</option></select>
...
<option>Blood</option><option>Leukopak</option><option>Sputum</option><option>Urine</option><option>Other</option></select>
...
<option>Ambient</option><option>Refrigerated</option><option>Other</option></select>
...
<option>Natasha.G</option><option>Drew.M</option><option>Lameez.P</option><option>Nthabiseng</option><option>Viola</option><option>Other</option></select>
...
<option value="">-- None Selected --</option><option>Testing</option><option>Storage</option><option>Disposed</option></select>
...
app.post("/login", (req, res) => {
  ...
  if(rolePasswords[role] && password === rolePasswords[role]) {
    // Redirect to the form with role as query
...
app.get("/form", (req, res) => {
  const role = req.query.role || 'site'; // default to site if missing
...
app.get("/form", (req, res) => {
  const role = req.query.role; // get role from login redirect
  if(!role || !["site","driver","lab"].includes(role)){
    return res.redirect("/login"); // prevent direct access without role
  }
  res.send(renderForm(role));
});
